# -------------------------------------------------------------------------------
# Name:        TheParser
# Purpose:     Top-down recursive descent parser.
#
# Author:      Oluwayomi Longe
#
# Created:     Fri 18-Nov-2016 20:39
...
import ast
from errors import InterpSyntaxError


class ParserError(InterpSyntaxError):
...
class Expression(Subparser):
    ...
    def parse(self, parser, tokens, precedence=0):
        ...
        print '..........Expression.parse(1): ', tokens.current(), subparser  # print
        if subparser is not None:
            print '..........Expression.inner.parse(2): ', tokens.current(), subparser  # print
            left = subparser.parse(parser, tokens)  # parsing "Print"
            print left
            print '..........Expression.inner.parse(3): ', tokens.current(), subparser  # (
            if left is not None:
...
class ExpressionStatement(Subparser):
    ...
    def parse(self, parser, tokens):
        print '..........ExpressionStatement.parse()'
        exp = Expression().parse(parser, tokens)
        if exp is not None:
            print '..........ExpressionStatement is not null!'
...
def main():
    ...
    for n, v, l, c in tokens:
        print """new Token ( "%s", "%s", %d, %d),""" % (n, v, l, c)
    print
    token_stream = lexer.TokenStream(tokens)
